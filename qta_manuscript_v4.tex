...
operating modes (enforced by interlocks IL-01, IL-02, and IL-14; see
...
NEG pump \designspec{}~\notinstalled{}, optical absorption $\eta_\mathrm{abs}$ \assumed{},
...
\noindent\textbf{Canonical source of truth.} These counts are computed by
\texttt{qta\_full\_sim.py} and recorded in \texttt{results\_gate\_table.csv}: 83
gates in total --- 0 PASS, 47 CONDITIONAL, 23 BLOCKED, 2 UNKNOWN, 11 DERIVED\_CHECK.
The canonical $\tau_c$ threshold is 292\,$\mu$s (v3.3); the earlier 27.728\,$\mu$s
value (v3.0) is SUPERSEDED, NOT\_CANONICAL, NOT\_LIVE\_GATE\_LOGIC.
...
\textbf{Correct category:} a blocked, conditional validation framework.
...
\midrule
A & BASELINE/STABILIZATION & Cryogenic baseline at the operating set-point; thermal, vacuum, and NV-baseline verification; readiness for any exposure or processing. Neither the sensing chain nor the LCVD process is active. \\
B & MATERIAL\_PROCESSING/LCVD\_GROWTH & LCVD active, with CH$_4$ or another precursor present; fs-laser processing and pulsed molecular-beam delivery. The sensing chain is inactive, and the sample stage is thermally isolated from the mixing chamber so that process heat is routed through the dedicated thermal dump path. No helium is admitted. \\
C & ISOLATION\_PURGE\_THERMAL\_RECOVERY & Growth inputs are stopped and the gas lines isolated; the cryopump and baffle remove residual species; the sample thermally relaxes and vibration settles; RGA, QCM, and witness-coupon checks are performed. \\
D & SENSING/MEASUREMENT & The LCVD process is inactive and the precursor is below threshold; He-3 is dosed; NV ODMR / Ramsey readout proceeds at the millikelvin sensing condition. \\
\bottomrule
\end{tabular}
\end{center}
...
QTA labels every item on \emph{three independent axes}. A component may carry one
label from each axis simultaneously; this is not a contradiction (e.g.\ a cryotrap
may be \designspec{} on the evidence axis and \texttt{NOT\_INSTALLED} on the
implementation axis).

\paragraph{Axis 1 --- parameter evidence status.}
\begin{description}
  \item[\assumed{}] Value assumed by the modeller; no external support.
  \item[\unknown{}] Too unknown to bound; no reliable forecast possible.
  \item[\mfrspec{} (\texttt{MANUFACTURER\_SPEC})] From a manufacturer datasheet.
  \item[\litbound{} (\texttt{LITERATURE\_BOUND})] Bounded by published literature.
  \item[\designspec{} (\texttt{DESIGN\_SPECIFIED})] Fixed by the design intent; not yet realised hardware.
  \item[\texttt{MEASURED\_EXTERNAL}] Measured, but on a different apparatus / not in this system.
  \item[\texttt{MEASURED\_IN\_SYSTEM}] Measured in this integrated system under relevant conditions.
\end{description}

\paragraph{Axis 2 --- implementation state.}
\begin{description}
  \item[\texttt{NOT\_INSTALLED}] Hardware not built/installed.
  \item[\texttt{INSTALLED\_NOT\_TESTED}] Installed but not yet exercised.
  \item[\texttt{TESTED}] Exercised in isolation.
  \item[\texttt{VERIFIED}] Verified in the integrated system.
\end{description}

\paragraph{Axis 3 --- gate verdict.}
\begin{description}
  ...
  \item[\textsc{Pass}] Allowed \emph{only} after verified in-system measurement (\texttt{MEASURED\_IN\_SYSTEM}) under relevant conditions.
\end{description}

\textbf{Absolute rule:} No gate reaches \textsc{Pass} from \assumed{},
\unknown{}, \litbound{}, \mfrspec{}, \designspec{}, \texttt{MEASURED\_EXTERNAL}, or
\texttt{NOT\_INSTALLED} inputs. \textsc{Pass} requires \texttt{MEASURED\_IN\_SYSTEM}
evidence and a \texttt{VERIFIED} implementation state.
...
$C_f = 2f_0^2/(\rho_q v_q) = 5.66\times10^6$\,Hz/(kg/m$^2$) = 0.057\,Hz/(ng/cm$^2$).
...
\begin{equation}
  ...
  \frac{\gamma_\mathrm{NV}^2\,\gamma_{^3\mathrm{He}}^2\,\hbar^2\,n_s\,\tau_c}{d^4}
  \propto \frac{S(\omega{=}0,\,\tau_c)}{d^4}
\end{equation}
...
At the dosing condition ($P = 1\,\mu$Pa), evaluated at the sensing-stage
temperature ($T_\mathrm{fridge} = 10$\,mK):
...
\subsection*{Gas-phase delivery constraints}
...
Reproduction instructions, output schema, and the numerical-methods description are
given in Appendix~\ref{app:sim} and in the repository files
\texttt{NUMERICAL\_METHODS.md}, \texttt{GATE\_TABLE\_README.md}, and
\texttt{VALIDATION\_PLAN.md}.

\subsection{Non-lumped 1D/2D reduced-order multiphysics layer}
\label{sec:multiphysics}

In addition to the lumped estimates above, QTA includes a spatially resolved 1D/2D
...
spatially resolved refinement; a 3D transient validation layer is implemented as
forecast-only numerical code (reduced-resolution, reduction-checked against the
1D/2D backends), is not hardware-validated, and introduces no PASS gates.} The layer is numerically self-checked (mesh convergence,
...
\paragraph{Numerical methods summary.} The backends and checks are, explicitly:
(i) a \emph{lumped state-vector} model (ordinary differential equations for node
temperatures/coverages) retained only as a fast comparator; (ii) a \emph{1D
finite-volume method-of-lines} thermal backend on a cell-centred mesh
($n_\mathrm{cells}=200$) integrated with a stiff BDF solver (\texttt{scipy
solve\_ivp}, \texttt{rtol}~$=10^{-6}$, \texttt{atol}~$=10^{-9}$); (iii) a \emph{2D
axisymmetric} $(r,z)$ refinement of the same physics with $2\pi r$ volume
weighting and a regularised symmetry axis at $r=0$; (iv) a 1D finite-volume
gas/contamination transport backend (upwind advection $+$ central diffusion,
method-of-lines, BDF) for CH$_4$/H$_2$/$^3$He/$^4$He. \emph{Boundary conditions:}
a Kapitza-type radiative backside sink
$-k\,\partial_z T|_L=\alpha_K(T_L^4-T_\mathrm{fridge}^4)$ ($\alpha_K$ \assumed{}),
an insulated/driven front face carrying the deposited flux, and zero-flux/symmetry
at $r=0$. \emph{Source terms:} averaged laser absorption $Q_\mathrm{laser}$,
microwave dissipation $Q_\mathrm{mw}$, background load $Q_\mathrm{bg}$, and, for gas
transport, inlet source, distributed pump sink, and cryobaffle capture.
\emph{Verification (numerical only):} mesh-convergence refinement, source/energy
conservation accounting, Kapitza-sign correctness, axis non-singularity, and a
2D$\rightarrow$1D reduction agreeing to $\sim$1.4\%. These are self-consistency
checks; no explicit/CFL-limited time stepping is used (the time integration is
implicit BDF), there is no COMSOL or other external solver and no hardware
validation; the additive 3D transient layer is forecast-only numerical code
(reduction-checked against the 1D/2D backends, never a hardware claim).
\textbf{Numerical self-consistency does not imply physical validation.}
...
\midrule
\multicolumn{4}{l}{\textbf{Full-table summary (\texttt{results\_gate\_table.csv}, single source of truth):}} \\
\multicolumn{4}{l}{0~\textsc{Pass} $|$ 47~\conditional{} $|$ 0~\textsc{Fail} $|$ 2~\unknown{} $|$ 23~\blocked{} $|$ 11~\derivedcheck{} \quad(83 gates total)} \\
\multicolumn{4}{l}{\footnotesize The rows above are a labelled \emph{subset} of critical gates; counts are over all 83 gates.} \\
\bottomrule
\end{longtable}
\end{center}
...
An LCVD breakthrough should not be claimed before this measurement is performed.
